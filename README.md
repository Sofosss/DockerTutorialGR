...
# Εισαγωγή στο Docker
##  Τι είναι το Docker 
Το Docker αποτελεί μια πλατφόρμα ανοικτού κώδικα που παρέχει ένα σύνολο εργαλείων για την αυτοματοποίηση της διαδικασίας ανάπτυξης, δοκιμών, εγκατάστασης, ρύθμισης και εκτέλεσης εφαρμογών μέσω της χρήσης ελαφρών (lightweight), φορητών εικονικών μηχανών, γνωστών ως "**containers**".


## Βασικά στοιχεία
1. **Docker Engine(Docker Client & Server)**
* Η αρχιτεκτονική του Docker, όπως αποτυπώνεται στην εικόνα που ακολουθεί, είναι client-server.
* Το/Τα Docker client(s) επικοινωνεί/ούν με τον Docker server(daemon), ο οποίος είναι υπεύθυνος για όλες τις επιμέρους λειτουργίες του Docker.
* Το Docker παρέχει ένα CLI (command line) εργαλείο, καθώς και ένα πλήρες RestFul API για την επικοινωνία με τον Docker daemon.
...
* Συνήθως οι εικόνες δημιουργούνται μέσω **Dockerfile** αρχείων. Πρόκειται για ένα είδος YAML αρχείου που περιγράφει τα βήματα για τη δημιουργία μιας Docker εικόνας. Περιέχει οδηγίες για την επιλογή της εικόνας βάσης, την εγκατάσταση εξαρτήσεων, τον καθορισμό του working directory και του σημείου εκκίνησης (entry point) κ.ά.
...
περιβάλλον, εξαλείφοντας με αυτόν τον τρόπο πιθανά προβλήματα ασυμβατότητας συστημάτων. Είναι, δηλαδή, σχεδιασμένα με τρόπο τέτοιο έτσι ώστε να μπορούν να αναπτυχθούν/εκτελεστούν σε έναν (οποιοδήποτε) προσωπικό υπολογιστή, σε έναν cluster στο Cloud, σε έναν virtual server και γενικότερα σε οποιοδήποτε υπολογιστικό σύστημα.

4. **Docker Registry**
* Το Docker αποθηκεύει και κατανέμει τα διάφορα Docker images σε αποθετήρια, τόσο δημόσια όσο και ιδιωτικά. 
* Η εταιρία Docker Inc. υποστηρίζει την κοινότητα διατηρώντας το Docker Hub, το μεγαλύτερο αποθετήριο για Docker images. Πρόκειται για μία διαδικτυακή υπηρεσία που παρέχει την δυνατότητα αποθήκευσης, διαμοιρασμού και αναζήτησης εικόνων Docker, τόσο δημόσιων όσο και ιδιωτικών.
* Υπάρχει, ωστόσο, και η δυνατότητα για κάποιον οργανισμό να υποστηρίξει ανεξάρτητο, ιδιωτικό Registry πίσω από το firewall του για την εσωτερική διαχείριση των Docker εικόνων. Αυτό σημαίνει ότι ο οργανισμός μπορεί να διατηρεί τις Docker εικόνες του (π.χ., εικόνες που χρησιμοποιούνται για τις εφαρμογές του) σε ένα ιδιωτικό αποθετήριο (Registry)  που είναι προσβάσιμο μόνο από το εσωτερικό δίκτυο του οργανισμού.
...
Για την εγκατάσταση του Docker engine στον προσωπικό σας υπολογιστή , θα πρέπει να ακολουθήσετε συγκεκριμένα βήματα, κατάλληλα προσαρμοσμένα στο λειτουργικό σύστημα που χρησιμοποιείτε.
...
Μόλις ολοκληρώσετε την εγκατάσταση του Docker, ελέγξτε ότι αυτή έγινε επιτυχημένα εκτελώντας την εντολή:
```bash
$ docker run hello-world
```
Προκειμένου να ελέγξετε την έκδοση του Docker που έχετε πλέον στον υπολογιστή σας, μπορείτε να χρησιμοποιήσετε την εντολή:
```bash
$ docker version
```
> [!WARNING]
> Επειδή το Docker χρησιμοποιεί ***Unix socket*** το οποίο ανήκει στον root χρήστη του υπολογιστή, το Docker CLI (Command Line) απαιτεί την χρήση του προσδιορισμού ***sudo***. Σε περίπτωση που κάτι τέτοιο δεν είναι επιθυμητό, χρειάζεται να προσθέσετε τον αντίστοιχο χρήστη σε ένα ***Unix group*** με την ονομασία **docker**. Η παραπάνω διαδικασία μπορεί να διεκπεραιωθεί μέσω της εκτέλεσης των παρακάτω εντολών:
...
# Ενεργοποίηση αλλαγών για τα groups κάνοντας relogin και έλεγχος  χρήσης docker χωρίς sudo
$ docker run hello-world
``` 
> [!CAUTION]
> Η εισαγωγή ενός χρήστη στο group Docker, του αναθέτει αυτόματα δικαιώματα επιπέδου διαχειριστή (root privileges). Επομένως, απαιτείται ιδιαίτερη προσοχή από τους χρήστες που εισέρχονται στο συγκεκριμένο group.
...
$ docker pull nginx 
```
κατεβάζουμε και αποθηκεύουμε την εικόνα nginx στον υπολογιστή μας.  
...
✨***docker ps***: Εμφανίζει μία λίστα με τα ενεργά containers μαζί με τις σχετικές πληροφορίες, όπως το container ID, το όνομα, την κατάσταση του container, τις πόρτες που χρησιμοποιεί κ.ά.
Για παράδειγμα, εκτελώντας, στον προσωπικό μας υπολογιστή την εντολή:
```bash 
$ docker ps 
```
...
```
εκκινείται το container ***"my_first_container"*** και τίθεται σε ενεργή κατάσταση που μπορεί να  επιβεβαιωθεί μέσω της εντολής **docker ps**.
...
✨***docker attach [OPTIONS] CONTAINER***: Χρησιμοποιείται για την σύνδεση του χρήστη με τα I/O streams του τερματικού ενός ενεργού container, επιτρέποντας την απευθείας επικοινωνία του με το περιβάλλον εκτέλεσης του container. Για παράδειγμα, εκτελώντας τις εντολές:
```bash 
...
> [!NOTE]
> Το αποτέλεσμα εκτέλεσης της εντολής docker attach μπορεί να μοιάζει κάποιες φορές με μη απόκριση του συστήματος, αλλά αυτό μπορεί να οφείλεται στην έλλειψη δεδομένων στο Ι/Ο stream του τερματικού στο οποίο συνδέθηκε.

> [!IMPORTANT]
> Επειδή με την εντολή **docker attach** γίνεται σύνδεση π.χ. στο stdout της διεργασίας του container, ο τερματισμός της τρέχουσας διεργασίας (με τη χρήση CTRL + C - σήμα SIGINT) ισοδυναμεί επίσης και με τον τερματισμό του container. 
...
✨***docker logs [OPTIONS] CONTAINER***: Χρησιμοποιείται για την εμφάνιση των δεδομένων κάποιου I/O stream της διεργασίας ενός ενεργού ή πρόσφατα τερματισμένου container, σύμφωνα με τον logging driver που έχει οριστεί για αυτό. Αν δεν επιλεγεί κάποια πιο σύνθετη ρύθμιση,
η εντολή δείχνει την έξοδο στο stdout της διεργασίας του container αυτού.

✨***docker rm [OPTIONS] CONTAINER [CONTAINER...]***: Χρησιμοποιείται για την διαγραφή ενός ή περισσότερων containers που έχουν τερματιστεί από το σύστημα. Χρησιμοποιείται, επομένως, για την αφαίρεση των container instances που δεν χρειάζεται πλέον ο χρήστης. Για παράδειγμα, εκτελώντας την εντολή:
```bash 
$ docker rm my_first_container 
```
θα διαγράψουμε το container με το όνομα **"my_first_container"**. Εάν θέλουμε να διαγράψουμε περισσότερα από ένα containers, τοποθετούμε τα ονόματά τους ως επιπλέον ορίσματα. Η διαγραφή του container μπορεί να επιβεβαιωθεί μέσω της εντολής **docker ps**.

> [!TIP]
> Χρησιμοποιώντας το flag -f στην εντολή **docker rm -f my_first_container** το container ***"my_first_container"*** θα διαγραφεί ακόμη και αν είναι ενεργό, αλλά αυτό ενδεχομένως να οδηγήσει σε απώλεια δεδομένων ή άλλα προβλήματα, αν δεν έχει εκτελεστεί προηγουμένως κατάλληλος έλεγχος.

# Κατασκευή εικόνας Docker μέσω Dockerfile

Όπως αναφέρθηκε νωρίτερα, ένα Dockerfile είναι ένα αρχείο τύπου YAML με ένα σύνολο από εντολές (instructions) μιας γλώσσας ειδικού πεδίου(Domain Specific Language) που κατευθύνουν τη δημιουργία μιας εικόνας Docker.  

Για να γίνει κατανοητός ο τρόπος με τον οποίο δομείται ένα τέτοιο αρχείο, θα δημιουργηθεί ένα [Dockerfile][dockerfile-cstd1] για την κατασκευή εικόνας η οποία θα περιγράφει το container μίας απλής [Web εφαρμογής][node-app]. Πιο συγκεκριμένα, θα γίνει deploy ένα container τοπικά, στο οποίο θα εκτελείται μία εφαρμογή γραμμένη με Express-Node.js, σκοπός της οποίας είναι η εξυπηρέτηση ενός endpoint. Στο  endpoint, η εφαρμογή θα δέχεται HTTP POST αιτήσεις με δεδομένα μορφής JSON, τα οποία ακολούθως θα αποθηκεύει στον server (σε τοπικό αρχείο τύπου JSON του container).  

Το αρχείο Dockerfile δομείται ως εξής:

```bash
# Χρησιμοποιούμε μια επίσημη εικόνα του Node.js ως βάση. Το tag latest δηλώνει ότι "κατεβάζουμε" την πιο πρόσφατα διαθέσιμη επίσημη εικόνα Node.js
FROM node:latest
...
```
> [!NOTE]
> Ο προσδιορισμός . ενημερώνει τον Docker daemon για το build context της εικόνας, δηλαδή το σύνολο των αρχείων και φακέλων που εντοπίζονται σε αυτό. Την πληροφορία αυτή την χρησιμοποιεί ο daemon μεταξύ άλλων για την πρόσβαση σε αρχεία που μπορεί να ορίζονται από εντολές στο Dockerfile καθώς και για την αποδοτική δημιουργία νέων εικόνων με την χρήση έξυπνων caching μηχανισμών.  
...
Τα Docker volumes είναι μηχανισμός που εισάγεται για την διαχείριση του ζητήματος αυτού, καθώς  τα volumes  παρακάμπτουν το ιεραρχικό σύστημα αρχείων. Πρόκειται για ειδικά ορισμένους καταλόγους που έχουν σκοπό τον διαμοιρασμό και την διατήρηση δεδομένων μεταξύ των διαφόρων containers, ανεξάρτητα από τον κύκλο ζωής τους. 

Στην δική μας εφαρμογή, o server αποθηκεύει τα δεδομένα των εισερχόμενων HTTP αιτήσεων σε ένα JSON αρχείο. Επειδή το αρχείο αυτό είναι αποθηκευμένο σε ένα στρώμα (layer) στο περιβάλλον του container, όταν το container διαγραφεί, θα διαγραφεί κι αυτό μαζί του. Συνεπώς, θα χάσουμε τα δεδομένα του server. Για να αποφευχθεί η ανεπιθύμητη αυτή κατάσταση, δημιουργούμε έναν καινούριο κατάλογο ***data_per*** και μέσω ενός Docker volume συνδέουμε (κάνουμε mount) τον τοπικό κατάλογό μας ***CaseStudy1/data_per*** στον κατάλογο ***opt/data_per*** του container, εκτελώντας την παρακάτω εντολή κατά τη δημιουργία του container:
...
```
Η τρίτη εντολή μπορεί να γραφτεί ισοδύναμα και ως εξής:

```bash
docker run -d --name express_server -p 8080:8891 -v "$(pwd)"/data_per:/opt/data_per nodejs_server:v0.1   
```

Με τον τρόπο αυτό δημιουργούμε ένα **bind mount**. Πρόκειται για μια τεχνική που χρησιμοποιείται συχνά από τους developers, ωστόσο η λειτουργία της στηρίζεται σε μεγάλο βαθμό στη δομή του συστήματος αρχείων του host με αποτέλεσμα να προκαλεί σημαντικά προβλήματα απόδοσης.  
...
```
ενώ η επισκόπηση των πληροφοριών ενός συγκεκριμένου volume είναι εφικτή με την εκτέλεση της εντολής:

```bash
docker volume inspect [OPTIONS] VOLUME [VOLUME...]
```
Η διαγραφή ενός volume, εφόσον δεν υπάρχει ενεργό container που το χρησιμοποιεί, πραγματοποιείται με την εκτέλεση της εντολής:

```bash
docker volume rm [OPTIONS] VOLUME [VOLUME...]
```
Τέλος, η διαγραφή όλων των αχρησιμοποίητων (unused) volumes του συστήματος πραγματοποιείται με την εκτέλεση της εντολής:
...
```
ενώ η επισκόπηση των πληροφοριών ενός συγκεκριμένου network είναι εφικτή μέσω της εντολής:

```bash
docker network inspect [OPTIONS] NETWORK [NETWORK...]
```
Η σύνδεση ενός container σε ένα συγκεκριμένο network είναι εφικτή μέσω της εντολής:

```bash
docker network connect [OPTIONS] NETWORK CONTAINER
```
Η αποσύνδεση ενός container από ένα network είναι εφικτή μέσω της εντολής:

```bash
docker network disconnect [OPTIONS] NETWORK CONTAINER
```
Η διαγραφή ενός συγκεκριμένου network, εφόσον δεν υπάρχει ενεργό container που  ανήκει σε αυτό, πραγματοποιείται με την εκτέλεση της εντολής:
...
Για να γίνει κατανοητός ο τρόπος με τον οποίο λειτουργεί ένα Docker Network, θα παρουσιάσουμε ένα παράδειγμα στο οποίο θα δημιουργείται ένα Docker Network με δύο containers. Το πρώτο container θα λειτουργεί ως server και θα έχει την λειτουργία που είχε και το αντίστοιχο container στο παράδειγμα με τα Volumes (Case Study 1). Το δεύτερο container θα λειτουργεί ως client. Μέσω κλήσης σε εξωτερικό API θα αποκτά ένα σύνολο δεδομένων, τα οποία στη συνέχεια θα στέλνει ανά τακτά χρονικά διαστήματα μέσω HTTP Requests στο server container. Όπως και στο Case study 1, το container του server θα αποθηκεύει τα δεδομένα αυτά σε ένα τύπου JSON αρχείο που είναι μόνιμο (persistent) εξαιτίας του volume που δημιουργήθηκε.

Τα Dockerfiles του συγκεκριμένου παραδείγματος βρίσκονται στον κατάλογο case_studies/CaseStudy2. 


Επισημαίνεται ότι η αλληλεπίδραση του χρήστη με τον Docker daemon καθίσταται δυνατή και μέσω Docker SDK (Software Develοpment  Kit) το οποίο παρέχει γλώσσες προγραμματισμού όπως η Python, οι οποίες μπορούν να χρησιμοποιηθούν για την διαχείριση και την αλληλεπίδραση με τα Docker containers. Μέσω του Docker SDK μπορούμε να εκτελέσουμε εντολές, να δημιουργήσουμε, να διαχειριστούμε και να παρακολουθήσουμε  containers, εικόνες και άλλα στοιχεία του Docker.
...
cd ../client
docker build -t ubuntu_client_image .
...
Παρατηρούμε ότι ο client αποκτά τα απαραίτητα δεδομένα και στη συνέχεια ανά συγκεκριμένες χρονικές στιγμές πραγματοποιεί HTTP αιτήσεις στον server παρέχοντάς του ως πληροφορία μερικές εγγραφές των δεδομένων του. Ο server λαμβάνει τις αιτήσεις του client και εγγράφει τις ληφθείσες εγγραφές σε ένα αρχείο στο volume.
 
Μέχρι τώρα είδαμε την επικοινωνία του Client με τον Docker daemon μέσω του CLI. Για λόγους πληρότητας, ακολούθως θα δείξουμε και ένα παράδειγμα που αξιοποιεί την δυνατότητα αλληλεπίδρασης με το Docker daemon μέσω ενός SDK στην Python. Πιο συγκεκριμένα, θα δημιουργήσουμε ένα Docker Network ως εξής:
...
Προκειμένου να χρησιμοποιήσουμε στην πράξη τα όσα αναφέρθηκαν  στο tutorial, θα εκτελέσουμε ένα τελικό case study (case_studies/CaseStudy 3). Θα δημιουργήσουμε τρία containers τα οποία θα ανήκουν στο ίδιο δίκτυο(network) και θα λειτουργούν συνεργατικά για να παρέχουν μια σχετικά απλή υπηρεσία.  
...
Η δημιουργία του container που υποστηρίζει την λειτουργία της βάσης δεδομένων της υπηρεσίας θα στηριχθεί στην εικόνα που δομείται βάσει του [Dockerfile][dockerfile-2]. Προφανώς, και αυτό το container θα ανήκει στο δίκτυο cstd3. Η κατασκευή της εικόνας και η δημιουργία του container εκτελείται ως εξής:
...
Τέλος, το τρίτο container που θα δημιουργήσουμε και  θα εντάξουμε στο δίκτυο cstd3 είναι αυτό του logger. Κατά τη δημιουργία του, θα του γίνουν mount όλα τα mounted volumes του server container. Η συγκεκριμένη ενέργεια  υπαγορεύεται από την  ανάγκη πρόσβασης του logger container στο αρχείο που εγγράφει ο server τις παρατηρήσεις του σχετικά με πιθανή ύποπτη κίνηση στην υπηρεσία. Η εικόνα του logger container βασίζεται στο [Dockerfile][dockerfile-3]. Η κατασκευή της εικόνας και η δημιουργία του logger container εκτελείται ως εξής:
...
Το παρακάτω gif παρουσιάζει βήμα προς βήμα την εκτέλεση όλων των ενεργειών που αναφέρθηκαν παραπάνω προκειμένου να γίνει έλεγχος της λειτουργίας της υπηρεσίας:
![](./images_media/case_study_3.gif)

Παρατηρούμε ότι η υπηρεσία μας λειτουργεί όπως επιθυμούσαμε. Όταν εντοπίζεται από το server ύποπτη κίνηση, την καταχωρεί σε ένα αρχείο το οποίο παρακολουθεί ο logger που με τη σειρά του την εμφανίζει στο τερματικό του. Επίσης, τα στοιχεία κάθε νέου  χρήστη αποθηκεύονται ορθά στη βάση δεδομένων του αντίστοιχου container. 
